...
function booleanFromEnv(name: string) {
  return z.union([z.boolean(), z.string()]).transform((value) => {
    ...
    throw new Error(`Invalid boolean value for ${name}: ${value}. Expected true/false, 1/0, yes/no, or on/off.`);
  });
}
...
const configSchema = z
  ...
  .transform((value) => ({
    ...value,
    TWITCH_EVENTSUB_AUTO_SYNC: value.TWITCH_EVENTSUB_AUTO_SYNC ?? false,
  }))
  .superRefine((value, ctx) => {
    if (!value.ERWIN_GATEWAY_ENABLED) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['ERWIN_GATEWAY_ENABLED'],
        message: 'ERWIN_GATEWAY_ENABLED=false rollback mode is no longer supported. erwin-gateway is required for Twitch transport.',
      });
    }
