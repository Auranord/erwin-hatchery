...
The player UI also includes a separate `Subscriber-Shop` box. It snapshots deterministic monthly pairings from `pet_species.is_shop_purchasable = true` and `hats.is_shop_purchasable = true`, each with stock 1 and a fixed 1-`voucher` cost. Purchases debit Gutscheine and create both an owned pet with the selected cosmetic hat equipped and a matching hat inventory slot in one ledgered transaction.
