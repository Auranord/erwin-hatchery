...
Reward mappings for egg rewards are authored by Hatchery from the admin panel rather than selected manually. `POST /api/admin/erwin-gateway/egg-rewards/sync` now treats gateway sync as discovery-only, then lists rewards, finds matching rewards by Twitch reward ID/title/app ownership key, adopts unowned manageable rewards through erwin-gateway, and only creates or mutates rewards when `canMutate` confirms Hatchery ownership. The current MVP beta egg uses gateway adoption identity `hatchery:basic_mystery_egg` / `basic_mystery_egg` while Hatchery keeps its economy mapping as `egg_type:beta_egg`. `gateway_reward_mappings` stores ownership key/status plus manageable/can-adopt/can-mutate flags so the admin UI can distinguish unowned, adopted, non-manageable, and other-app-owned rewards. Unknown rewards are stored with `mapping_status='unknown'` for diagnostics and are ignored safely rather than crashing or mutating inventory.
