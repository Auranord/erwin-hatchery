...
- Use `pet_species` for species templates/default stats and species default abilities, and `pets` for owned pet instances with permanent base stats derived from species defaults plus hatch variance plus an individual ability copied from the species at hatch.
...
- Each pet has exactly one class, one element, one individual ability, and a trait list via `pet_trait_assignments`. Each trait defines positive or negative modifiers for every base stat. Each pet may equip one cosmetic hat; hats never affect combat stats.
