...
Duplicate pet training adds server-authoritative pet progression columns: `training_points`, `level_bonus_hp`, `level_bonus_atk`, `level_bonus_def`, `level_bonus_spd`, `level_bonus_gain`, `level_bonus_pow`, `is_locked`, `status`, `consumed_by_pet_id`, and `consumed_at`. Active collection queries use `status = active`; consumed pets remain immutable audit/material history rows. Each training action writes one `economy_ledger` row with target, consumed pet IDs, awarded points, before/after level, stat bonuses, and stat changes.
