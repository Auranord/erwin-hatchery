...
Seed `pet_rarities`, `pet_classes`, `elements`, `pet_abilities`, and `hats` before seeding owned pet fixtures. Rarity seed values define display/economy/combine/recycle metadata only, never stat multipliers. Seeded class counters are: `protector`/Beschützer targets enemy ATK, `sunderer`/Spalter targets enemy DEF, `saboteur`/Saboteur targets enemy SPD, `drainer`/Entlader targets enemy GAIN, and `nullifier`/Bannbrecher targets enemy POW.
