...
Tracks the standard incubator queue and row-upgraded queue slots.

```text
id uuid primary key
owner_user_id uuid references users(id)
slot_source text not null -- default, upgrade; subscriber/admin sources are not used in MVP
...
- Ensure one default standard incubator queue slot exists for every player.
- Additional incubator queue slots are player-purchased inventory row upgrades; each upgrade adds one `upgrade` queue slot and is ledgered as an `inventory_row_upgraded` economy event.
...
The seeded stat-tradeoff consumables cover every ordered pair among HP, ATK, DEF, SPD, GAIN, and POW. Each sweet increases one stat by one stat point and decreases a different stat by one stat point. Their `config` uses `{ target: 'pet', duration: 'permanent', statModifiers: { [increased_stat]: amount, [decreased_stat]: -amount } }`, where HP stat points are stored as +/-10 HP and non-HP stat points are stored as +/-1; display names and descriptions should hint at the effect without being named directly after stat codes. All seeded stat-tradeoff consumables carry future shop metadata of `resource_price = 100` cracked eggs, `stock = 25`, and `is_shop_purchasable = true`.
...
Egg reward mappings are created from Hatchery-owned `egg_types` via `POST /api/admin/erwin-gateway/egg-rewards/sync`. The sync discovers gateway rewards first, adopts matching unowned manageable rewards, and only creates/updates rewards after erwin-gateway reports that Hatchery can mutate them. `gateway_reward_mappings.local_reward_type` remains the Hatchery economy type (`egg_type:<egg_type_id>`), and the table also stores `app_ownership_key`, `ownership_status`, `manageable`, `can_adopt`, and `can_mutate` for admin diagnostics and mutation gating. The MVP beta egg binds to gateway `local_reward_type='basic_mystery_egg'` and `app_ownership_key='hatchery:basic_mystery_egg'`. The MVP seed creates active `beta_egg` plus inactive `starter_egg`; other old non-MVP egg types are deactivated or removed when unreferenced by migration `0018_beta_egg_authority`.
