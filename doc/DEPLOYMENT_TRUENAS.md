...
Egg reward mappings are authored from Hatchery `egg_types`. The admin gateway egg sync discovers existing gateway/Twitch rewards, adopts matching unowned manageable rewards, and only creates/updates rewards that erwin-gateway marks as mutable by Hatchery. It stores `gateway_reward_mappings.local_reward_type = 'egg_type:<egg_type_id>'` plus ownership diagnostics (`app_ownership_key`, `ownership_status`, `manageable`, `can_adopt`, `can_mutate`) and mirrors `egg_types.is_active` only after adoption/mutation is allowed. The MVP seed creates active `beta_egg` for Channel Point redemption plus inactive `starter_egg` for each player's first egg; unknown/unowned rewards are stored for diagnostics and ignored safely.
