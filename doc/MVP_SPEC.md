...
The player UI also includes a separate mobile-first `Subscriber-Shop` box. It lists monthly pet-and-hat pair offers built from the cross-product of active `pet_species` and `hats` rows where both sides have `is_shop_purchasable = true`. The monthly selection is deterministic for the UTC month key and is snapshotted in `shop_offer_selections` when generated, so newly added pets or hats do not change the current month's contents. `SUBSCRIBER_SHOP_MONTHLY_OFFER_COUNT` configures how many pet-hat pairs appear each month and defaults to 3.
