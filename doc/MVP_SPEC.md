...
The player UI includes a mobile-first `Shop` box that lists weekly offers for purchasable gem equipment and consumables. Only active records with `is_shop_purchasable = true`, a positive `resource_price`, and positive `stock` can be selected when a new weekly offer set is first generated. The selected offers are then snapshotted in `shop_offer_selections` for the UTC Monday week key, so adding new pets, hats, consumables, or equipment later does not change the already-visible offer set. `SHOP_WEEKLY_EQUIPMENT_OFFER_COUNT` and `SHOP_WEEKLY_CONSUMABLE_OFFER_COUNT` configure how many equipment gems and consumables appear each week; both default to 5.
...
### Duplicate pet training

Players may train one active hatched pet by consuming one or more owned active duplicates of the same species. The target cannot consume itself. Materials must not be favorited/locked, selected for battle, already consumed, scrapped, or part of an unresolved battle event. Consumed pets are not deleted; they are marked `status = consumed`, linked to the target with `consumed_by_pet_id`, timestamped with `consumed_at`, and removed from active inventory. Training is fully server-authoritative and transactional.

Training uses integer training points. A level-0 material grants 2 points, level 1 grants 3, and level N >= 2 grants 2^N. Level N -> N+1 costs 2^(N+1) points, excess carries over, and the default configurable max level is 10. Level bonuses use the pet class main stat (+2 stat points per level) and two secondary stats (+1 stat point per level each); each HP stat point is worth +10 HP, so HP gains are +20 HP for a main-stat level and +10 HP for a secondary-stat level. Original rolled base stats stay separate.
