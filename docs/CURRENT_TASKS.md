...
- Gateway subscription, resub, gift-sub, subscription-end, and Bits/cheer events are durably stored and deduped. Active mode applies only fixed Gutschein effects for identifiable users; subscription events do not derive local subscriber status, and anonymous paid events are audited without gifter credit.
